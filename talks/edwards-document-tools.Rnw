...
\begin{enumerate}
  \item Scoping -- decide what information to include, layout, format, different
  formats for different situations?, etc. etc. \textcolor{red}{{Start in workshop,
      will evolve with further feedback, to be finalised before step 2}}
  \item Software -- small team to develop package
  \item Assessment analysts would have get their results into a standard format
\end{enumerate}
}
%----------------------------------------------------------

\frame{\frametitle{Software development}

A small team to develop the package, based on concepts learned in previous
