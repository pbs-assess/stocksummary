...
\item We have templates for building properly-formatted:
  ...
  \ei
\item These are built into our {\tt csasdown} R package.
\item So we know how to develop templates.
\item Already extensively use our {\tt rosettafish} R package to translate figures.
\ei
...
For simpler assessment and anyone not using R, can still use the Word template.
}
%----------------------------------------------------------
